...
> A TypeScript rewrite of Hello Minecraft! Launcher (Electron + React).
>
> Hello Minecraft! Launcher 的 TypeScript 重写版（Electron + React）。
...
HMCL Rewrite 是 Hello Minecraft! Launcher (HMCL) 的 TypeScript 重写版：Electron
主进程 + Web 前端，核心逻辑位于框架无关的 `@hmcl/core` 库，UI 与交互尽量贴合
原版 HMCL 的 Material Design 风格。
...
HMCL Rewrite is a TypeScript rewrite of Hello Minecraft! Launcher (HMCL),
built with an Electron main process and a React web frontend. All core logic
lives in the framework-agnostic `@hmcl/core` package, and the UI mirrors the
original HMCL's Material Design look and feel.
...
> **状态 / Status**: 骨架 + 游戏启动 / 下载核心模块已完成，支持离线账户与一键启动。
> 加载器安装（Forge / Fabric / NeoForge）、微软 OAuth、Java 自动下载尚未实现
> （见路线图 / roadmap）。
>
> The skeleton and game-launch/download core modules are done, with offline
> accounts and one-click launch supported. Loader installers
...
- UI：实例列表 / 版本下载 / 设置 / 启动日志，Material 3（monet）主题，
  ...
  with background image and transparent-window support
- / 支持背景图与透明窗口模式
...
### 与原版 HMCL 的对应关系 / Correspondence with original HMCL

| 本项目 This repo | HMCL (Java) | 忠实度 Fidelity |
|---|---|---|
...
## 已知限制 / Known Limitations (Roadmap)

1. **Mod 加载器安装**：Forge / Fabric / NeoForge 安装器未实现（启动已兼容其版本 JSON）。
   **Loader installers** not implemented (launch already handles their version JSON).
2. **微软登录**：仅离线档案；OAuth 设备码流程待接入。
   Microsoft login is offline-only; device-code OAuth flow pending.
3. **Java 自动下载**：目前只做本机检测。
   Only local Java detection for now.
4. **断点续传 / ETag 缓存**：下载器暂用整文件重试。
   Downloader retries whole files; no resume / ETag caching yet.
5. **整合包**：CurseForge / Modrinth 导入待实现。
   Modpack import (CurseForge / Modrinth) pending.
